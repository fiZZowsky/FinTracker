...
namespace FinTracker.Tests.Controllers
{
    public class ReceiptsControllerTests
    {
        ...
        public async Task GetReceiptById_ShouldReturnOk_WhenReceiptExists()
        {
            ...
            _receiptServiceMock.Setup(x => x.GetByIdAsync(receiptId, It.IsAny<string?>()))
                .ReturnsAsync(dto);

            var result = await _controller.GetReceiptById(receiptId, null);
        ...
        {
            _receiptServiceMock.Setup(x => x.GetByIdAsync(It.IsAny<int>(), It.IsAny<string?>()))
                .ReturnsAsync((ReceiptDTO?)null);

            var result = await _controller.GetReceiptById(999, null);
